...
from hongtu.my_sageconv_with_hp import SAGEConvHP
...
class SAGE(nn.Module):
    def __init__(self, in_size, hid_size, out_size, proc_id, device, nprocs):
        super().__init__()
        self.layers = nn.ModuleList()
        # three-layer GraphSAGE-mean
        self.layers.append(SAGEConvHP(in_size, hid_size, "mean", proc_id, device, nprocs))
        self.layers.append(SAGEConvHP(hid_size, hid_size, "mean", proc_id, device, nprocs))
        self.layers.append(SAGEConvHP(hid_size, out_size, "mean", proc_id, device, nprocs, final_layer=True))
...
def run(proc_id, nprocs, devices, g, data, mode, num_epochs):
    ...
    model = SAGE(in_size, 256, num_classes, proc_id, device,nproc).to(device)
